...
async def list_orders(user: CustomerDep) -> list[OrderSummaryOut]:
    """
...
async def get_order(order_id: str, user: CustomerDep) -> OrderDetailOut:
...
async def cancel_order(order_id: str, user: CustomerDep) -> None:
    service.cancel(user.db, user.id, order_id)


@router.post("/{order_id}/advance", status_code=status.HTTP_204_NO_CONTENT)
async def advance_stage(order_id: str, body: AdvanceStageIn, user: CustomerDep) -> None:
