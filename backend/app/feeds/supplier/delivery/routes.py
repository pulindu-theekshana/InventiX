...
async def get_queue(user: SupplierDep) -> list[StageGroupOut]:
    return service.queue(user.db, user.id)


@router.post("/{order_id}/advance", status_code=status.HTTP_204_NO_CONTENT)
async def advance_stage(order_id: str, body: AdvanceIn, user: SupplierDep) -> None:
...
async def mark_delivered(order_id: str, user: SupplierDep) -> None:
